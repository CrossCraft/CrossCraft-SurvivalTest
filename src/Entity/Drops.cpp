...
namespace CrossCraft {
    ...
    void Drops::add_drop(DropData data) { drops.push_back(data); }
    ...
    auto Drops::setup_model(uint8_t type) -> void {
        ...
        if (type == 6 || type == 37 || type == 38 || type == 39 || type == 40) {
            ...
            add_face_to_mesh(bottomFace, Chunk::getTexCoord(type, LIGHT_BOT),
                             LIGHT_BOT, p, type);
            add_face_to_mesh(leftFaceHalf, Chunk::getTexCoord(type, LIGHT_SIDE_X),
                             LIGHT_SIDE_X, p, type);
            add_face_to_mesh(rightFaceHalf, Chunk::getTexCoord(type, LIGHT_SIDE_X),
                             LIGHT_SIDE_X, p, type);
            add_face_to_mesh(frontFaceHalf, Chunk::getTexCoord(type, LIGHT_SIDE_Z),
                             LIGHT_SIDE_Z, p, type);
            add_face_to_mesh(backFaceHalf, Chunk::getTexCoord(type, LIGHT_SIDE_Z),
                             LIGHT_SIDE_Z, p, type);
        } else {
            ...
            add_face_to_mesh(leftFace, Chunk::getTexCoord(type, LIGHT_SIDE_X),
                             LIGHT_SIDE_X, p, type);
            add_face_to_mesh(rightFace, Chunk::getTexCoord(type, LIGHT_SIDE_X),
                             LIGHT_SIDE_X, p, type);
            add_face_to_mesh(frontFace, Chunk::getTexCoord(type, LIGHT_SIDE_Z),
                             LIGHT_SIDE_Z, p, type);
            add_face_to_mesh(backFace, Chunk::getTexCoord(type, LIGHT_SIDE_Z),
                             LIGHT_SIDE_Z, p, type);
        }
