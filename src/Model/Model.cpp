...
namespace CrossCraft
{
    ...
    void Model::load(YAML::Node node, glm::vec2 texSize)
    {
        ...
        auto anchorPos = glm::vec3(anchor[0].as<float>(), anchor[1].as<float>(), anchor[2].as<float>());
        auto sizeVec = glm::vec3(size[0].as<float>(), size[1].as<float>(), size[2].as<float>());

        auto uv = node["uv"];

        // You start furthest from the anchor position to grow the mesh around said anchor
