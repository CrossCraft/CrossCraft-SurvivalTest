...
namespace CrossCraft
{
    ...
    void Pig::draw(PigData *sd)
    {
        ...
        ctx->matrix_clear();
        ctx->matrix_translate({sd->pos.x, sd->pos.y - 0.125f, sd->pos.z});
        ...
        leg.draw({0.3125f, -0.5f, 0.1875f}, {0, 180, 0}, {-1, 1, 1});
        leg.draw({0.3125f, -0.5f, -0.1875f }, {0, 0, 0}, {1, 1, 1});
        leg.draw({1.0625f, -0.5f, 0.1875f}, {0, 180, 0}, {-1, 1, -1});
        leg.draw({1.0625f, -0.5, -0.1875f}, {0, 0, 0}, {1, 1, -1});
        ctx->matrix_clear();
    }
} // namespace CrossCraft